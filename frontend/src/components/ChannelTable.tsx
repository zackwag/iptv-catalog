...
export default function ChannelTable({
  ...
  if (channels.length === 0 && pinnedChannels.length === 0) {
  ...
  function renderRow(ch: Channel, pinned = false) {
    const selected = selectedIds.has(ch.id);
    const categories = ch.categories ? ch.categories.split(",").filter(Boolean) : [];

    return (
      <tr
        key={(pinned ? "pinned-" : "") + ch.id}
